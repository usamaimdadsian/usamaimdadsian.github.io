...
import { getData, styles } from "./data";
    ...
    <div>
      <div ref={elementRef} style={styles.resumeContent}>
        <div style={styles.title}>
          <h1>{data.name}</h1>
          <p>{data.location} | {data.number} | <a href={"mailto:"+data.email}>{data.email}</a> | <a href={data.linkedinLink}>{data.linkedinName}</a></p>
        </div>
        <main>
          {data.sections.map((sec,index) => (
            <React.Fragment key={index}>
      ...
      <button style={styles.downloadBtn} disabled={downloading} onClick={generatePdf}>
